...
func setAnonAccess(ctx context.Context, q querier, level domain.AccessLevel) error {
	_, err := q.ExecContext(ctx,
		`UPDATE settings SET value = ?, updated_at = CURRENT_TIMESTAMP WHERE key = 'anon_access'`,
		level.String())
	return err
}
...
func setAllowKeylessAccess(ctx context.Context, q querier, allow bool) error {
	_, err := q.ExecContext(ctx,
		`UPDATE settings SET value = ?, updated_at = CURRENT_TIMESTAMP WHERE key = 'allow_keyless'`,
