...
func getRepoByName(ctx context.Context, q querier, name string) (*domain.Repo, error) {
	row := q.QueryRowContext(ctx, `SELECT `+repoColumns+` FROM repos WHERE name = $1`, name)
	r, err := scanRepo(row)
...
func listReposByIdentityID(ctx context.Context, q querier, identityID string) ([]*domain.Repo, error) {
	rows, err := q.QueryContext(ctx, `SELECT `+repoColumns+` FROM repos WHERE identity_id = $1`, identityID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func updateRepo(ctx context.Context, q querier, repo *domain.Repo) error {
	_, err := q.ExecContext(ctx,
		`UPDATE repos SET name = $1, project_name = $2, description = $3, private = $4, mirror = $5, hidden = $6, identity_id = $7, updated_at = NOW()
		 WHERE id = $8`,
...
func deleteRepoByName(ctx context.Context, q querier, name string) error {
	_, err := q.ExecContext(ctx, `DELETE FROM repos WHERE name = $1`, name)
	return err
}
