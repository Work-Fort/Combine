...
func listReposByUserID(ctx context.Context, q querier, userID int64) ([]*domain.Repo, error) {
	rows, err := q.QueryContext(ctx, `SELECT `+repoColumns+` FROM repos WHERE user_id = $1`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func updateRepo(ctx context.Context, q querier, repo *domain.Repo) error {
	_, err := q.ExecContext(ctx,
		`UPDATE repos SET name = $1, project_name = $2, description = $3, private = $4, mirror = $5, hidden = $6, user_id = $7, updated_at = NOW()
		 WHERE id = $8`,
