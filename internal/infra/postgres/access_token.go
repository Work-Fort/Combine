...
func getAccessToken(ctx context.Context, q querier, id int64) (*domain.AccessToken, error) {
	row := q.QueryRowContext(ctx, `SELECT `+accessTokenColumns+` FROM access_tokens WHERE id = $1`, id)
...
func getAccessTokenByToken(ctx context.Context, q querier, token string) (*domain.AccessToken, error) {
	row := q.QueryRowContext(ctx, `SELECT `+accessTokenColumns+` FROM access_tokens WHERE token = $1`, token)
	t, err := scanAccessToken(row)
	if err == sql.ErrNoRows {
...
func listAccessTokensByUserID(ctx context.Context, q querier, userID int64) ([]*domain.AccessToken, error) {
	rows, err := q.QueryContext(ctx, `SELECT `+accessTokenColumns+` FROM access_tokens WHERE user_id = $1`, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func createAccessToken(ctx context.Context, q querier, name string, userID int64, token string, expiresAt time.Time) (*domain.AccessToken, error) {
	...
	if expiresAt.IsZero() {
		err = q.QueryRowContext(ctx,
			`INSERT INTO access_tokens (name, user_id, token, updated_at) VALUES ($1, $2, $3, NOW()) RETURNING id`,
			name, userID, token).Scan(&id)
	} else {
		err = q.QueryRowContext(ctx,
			`INSERT INTO access_tokens (name, user_id, token, expires_at, updated_at) VALUES ($1, $2, $3, $4, NOW()) RETURNING id`,
			name, userID, token, expiresAt.UTC()).Scan(&id)
	}
	if err != nil {
...
func deleteAccessToken(ctx context.Context, q querier, id int64) error {
	_, err := q.ExecContext(ctx, `DELETE FROM access_tokens WHERE id = $1`, id)
	return err
}
