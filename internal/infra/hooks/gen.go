...
func GenerateHooks(_ context.Context, cfg *config.Config, repo string) error {
	...
	for _, hook := range []string{
		...
		// Write the combine hook inside ${hook}.d directory.
		hp = filepath.Join(hp, "combine")
...
# AUTO GENERATED BY COMBINE, DO NOT MODIFY
data=$(cat)
exitcodes=""
hookname=$(basename $0)
...
// hooksTmpl is the combine hook that will be run by the git hooks
// inside the hooks directory.
var hooksTmpl = template.Must(template.New("hooks").Parse(`#!/usr/bin/env bash
# AUTO GENERATED BY COMBINE, DO NOT MODIFY
if [ -z "$COMBINE_REPO_NAME" ]; then
	echo "Warning: COMBINE_REPO_NAME not defined. Skipping hooks."
	exit 0
fi
